...
import { FocusChangeBackgroundType, FocusMoveType, GridConfig, PageConfig } from "./type";
  ...
  initTimeLine() {
    ...
    if (this.focus) {
      if (this.focus.type.type == "move") {
        ...
        timeline.from(this.focus.grid, { duration: 0.2, x: x }, "pagestart");
      }
    }
    ...
    if (this.focus && (this.focus.type.type == "move")) {
      timeline.call(() => { this.focus!.grid.updateBackground(GridBackground) });
    }
